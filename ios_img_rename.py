...
class App(Frame):
	...
	FILE_PATTERN = re.compile(r'^(\d+)(_\d+_iOS\.(?:jpg|jpeg|png))$')
	...
	def dir_selection(self):
		...
		self.desired_dir = desired_dir = \
			filedialog.askdirectory(initialdir=App.DIRECTORY, title='Please select a directory to check for')

		if desired_dir is None or not desired_dir: # empty string
	...
	def rename(self):
		...
		for file in target_files:
			...
			new_name = os.path.join(target_dir, newdate_prefix + pattern_result.group(2))
